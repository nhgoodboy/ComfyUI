...
class StyleService:
    ...
    async def validate_style(self, style_id: str) -> bool:
        """验证风格是否存在"""
        try:
            return self.style_registry.style_exists(style_id)
        except Exception as e:
    ...
    async def search_styles(self, query: str) -> List[StyleInfo]:
        """搜索风格"""
        try:
            return self.style_registry.search_styles(query)
        except Exception as e:
    ...
    async def get_style_count(self) -> int:
        """获取风格数量"""
        try:
            return self.style_registry.get_style_count()
        except Exception as e:
    ...
    async def get_workflow_file(self, style_id: str) -> Optional[str]:
        """获取工作流文件路径"""
        try:
            return self.style_registry.get_workflow_file(style_id)
        except Exception as e:
    ...
    async def reload_styles(self):
        """重新加载风格配置"""
        try:
            self.style_registry.reload_styles()
