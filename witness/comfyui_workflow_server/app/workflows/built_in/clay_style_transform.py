...
class ClayStyleTransformWorkflow(BaseWorkflow):
    """黏土风格转换工作流
    ...
    def get_metadata(self) -> WorkflowMetadata:
        ...
        return WorkflowMetadata(
            ...
            name="黏土风格转换",
            description="将输入图像转换为黏土风格的图像，使用Flux模型和ControlNet技术",
            version="1.0.0",
            workflow_type=WorkflowType.IMAGE_TO_IMAGE,
            author="ComfyUI工作流服务器",
            tags=["图像处理", "黏土风格", "风格转换", "AI绘画", "Flux模型"],
            parameters=[
                WorkflowParameter(
                    name="image_url",
                    type="string",
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"黏土风格转换参数验证完成: image_url={image_url}")
    ...
    async def pre_process(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 上传到ComfyUI
            filename = await service.upload_image(image_data, "clay_input.jpg")
    ...
    async def build_workflow(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "1" in workflow and workflow["1"].get("class_type") == "LoadImage":
            image_filename = parameters.get("image_filename", "clay_input.jpg")
        ...
        if "35" in workflow and workflow["35"].get("class_type") == "SaveImage":
            workflow["35"]["inputs"]["filename_prefix"] = "clay_style"
            self.logger.debug("设置输出文件前缀: clay_style")
        
        self.logger.info(f"黏土风格工作流构建完成，包含 {len(workflow)} 个节点")
        return workflow
    ...
    def _get_workflow_template(self) -> Dict[str, Any]:
        ...
        try:
            ...
            template_path = os.path.join(
                current_dir, 
                "../../../workflows/clay_style_transform.json"
            )
            
            # 规范化路径
            template_path = os.path.normpath(template_path)
            
            self.logger.info(f"加载黏土风格工作流模板: {template_path}")
            
            # 读取JSON文件
            with open(template_path, 'r', encoding='utf-8') as f:
                template = json.load(f)
            
            self.logger.info(f"成功加载黏土风格工作流模板，包含 {len(template)} 个节点")
            return template
            
        except FileNotFoundError:
            self.logger.error(f"黏土风格工作流模板文件未找到: {template_path}")
            raise FileNotFoundError(f"黏土风格工作流模板文件不存在: {template_path}")
        except json.JSONDecodeError as e:
            self.logger.error(f"黏土风格工作流模板JSON解析失败: {e}")
            raise ValueError(f"黏土风格工作流模板JSON格式错误: {e}")
        except Exception as e:
            self.logger.error(f"加载黏土风格工作流模板时发生错误: {e}")
            raise RuntimeError(f"加载黏土风格工作流模板失败: {e}")
    ...
    async def post_process(self, workflow_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            processed_result = {
                ...
                "metadata": {
                    "style": "clay",
                    "workflow_type": "clay_style_transform"
                }
            }
                ...
                for image_info in workflow_result["images"]:
                    ...
                    if filename:
                        ...
                        processed_result["output_images"].append({
                            "filename": filename,
                            "url": image_url,
                            "type": "clay_style_image"
                        })
                        self.logger.debug(f"处理输出图片: {filename}")
            
            self.logger.info(f"黏土风格转换后处理完成，生成 {len(processed_result['output_images'])} 张图片")
            return processed_result
            
        except Exception as e:
            self.logger.error(f"黏土风格转换后处理失败: {e}")
            # 返回原始结果，避免完全失败
