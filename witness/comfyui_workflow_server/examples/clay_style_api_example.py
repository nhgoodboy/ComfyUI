...
class ClayStyleAPIClient:
    ...
    async def execute_clay_style_transform(self, image_url: str) -> str:
        """执行黏土风格转换"""
        payload = {
        ...
        async with self.session.post(
            f"{self.base_url}/api/v1/workflows/clay_style_transform/execute",
            json=payload,
...
async def main():
    ...
    async with ClayStyleAPIClient() as client:
        try:
            # 1. 健康检查
            print("1. 检查服务健康状态...")
            ...
            if workflows.get("success"):
                ...
                if "clay_style_transform" not in available_workflows:
                    print("   ❌ 黏土风格转换工作流不可用")
                    return
            else:
            ...
            # 3. 获取工作流详情
            print("\n3. 获取黏土风格转换工作流详情...")
            workflow_info = await client.get_workflow_info("clay_style_transform")
            ...
            task_id = await client.execute_clay_style_transform(test_image_url)
